...
class RobloxStatusMonitor:
    def __init__(self, dig_tool_instance=None):
        ...
        self.is_joining = self.is_playing = self.is_disconnected = self.is_game_left = (
            self.is_roblox_closed
        ) = False
        ...
        with self._notification_lock:
            try:
                ...
                message_key = (
                    ...
                    if any(
                        x in message
                        for x in ["Disconnected", "Kicked", "Timeout", "Banned"]
                    )
                    else (
                        "rejoining"
                        if "Auto-rejoining" in message
                        else (
                            "rejoin_success"
                            if "Auto-rejoin successful" in message
                            else (
                                "rejoin_failed"
                                if "Auto-rejoin failed" in message
                                else (
                ...
                if (
                    message_key in self._last_notification_time
                    and current_time - self._last_notification_time[message_key]
                ...
                if webhook_url and webhook_url.strip():
                    ...
                    user_id = (
                        get_param(self.dig_tool, "user_id") if include_user else None
                    )
    ...
    def get_latest_log_file(self):
        ...
        for pattern in self.file_patterns:
            for log_file in self.log_path.glob(pattern):
                if (
                    log_file.is_file()
                    and (file_time := log_file.stat().st_mtime) > latest_time
                ):
    ...
    def process_log_file(self):
        ...
        if not self.current_log_file or not get_param(
            self.dig_tool, "auto_rejoin_enabled"
        ):
            return
        try:
            with open(
                str(self.current_log_file), "r", encoding="utf-8", errors="ignore"
            ) as f:
        ...
        if self.joining_keyword in line:
            ...
            if hasattr(self, "_recently_rejoined") and self._recently_rejoined:
                self.send_discord_notification(
                    "✅ **Successfully rejoined**: Now connected to server",
                    color=0x2ED573,
                )
                self._recently_rejoined = False
                if hasattr(self.dig_tool, "rejoiner") and hasattr(
                    self.dig_tool.rejoiner, "_current_attempt_start"
                ):
            ...
            reason = (
                (match := re.search(self.disconnect_regex, line))
                and match.group(1)
                or "N/A"
            )
            ...
            disconnect_messages = {
                "17": "🔌 **Disconnected**: Lost connection to server",
                "267": "⚠️ **Kicked**: You were kicked from the server",
                "279": "⏰ **Timeout**: Connection timed out",
                "529": "🚫 **Banned**: You have been banned from this server",
                "773": "🔄 **Teleport**: Server is teleporting players",
            }
            self.send_discord_notification(
                disconnect_messages.get(
                    reason, f"❌ **Disconnected**: Reason code {reason}"
                ),
                color=0xFF4757,
            )
    ...
    def start_file_watcher(self):
        ...
        self.log_observer.schedule(
            self.observer_handler, path=self.log_path, recursive=False
        )
    ...
    def monitor_log_files(self):
        ...
        while not self._stop_event.is_set():
            ...
            if (latest_file := self.get_latest_log_file()) and str(latest_file) != str(
                ...
                try:
                    with open(
                        ...
                        if old_file is None:
                            ...
                            logger.info(
                                f"Processing {len(lines)} lines from new log file"
                            )
                            for line in lines:
                                if line.strip():
                ...
                if old_file:
                    self.reset_status(
                        clear_notifications=False, preserve_automation_state=True
                    )
                next_interval = 0.5
    ...
    def can_rejoin(self):
        return (
            ...
            or self.is_game_left
            or self.is_roblox_closed
            or not self.is_roblox_running()
        )
            ...
            if hasattr(self.dig_tool, "running") and self.dig_tool.running:
                ...
                if hasattr(self.dig_tool, "update_status"):
                    self.dig_tool.update_status(
                        "Stopped (Disconnected) - Auto-rejoin monitoring..."
                        if auto_rejoin_enabled
                        else "Stopped (Disconnected)"
                    )
    ...
    def _resume_automation_after_rejoin(self, delay=None):
        ...
        logger.debug(f"Resuming automation after {delay}s delay")

        try:

            def delayed_restart():
                logger.debug(f"Starting delayed restart (waiting {delay}s)")
                time.sleep(delay)
                logger.debug("Delay completed, focusing Roblox and re-equipping shovel")
                ...
                try:
                    ...
                    time.sleep(0.5)
                    logger.debug("Shovel re-equipped successfully")
                except Exception as e:
                    logger.debug(f"Error re-equipping shovel: {e}")

                if hasattr(self.dig_tool, "running") and not self.dig_tool.running:
                    automation_manager = getattr(
                        self.dig_tool, "automation_manager", None
                    )
                    if automation_manager and hasattr(
                        automation_manager, "restart_automation"
                    ):
                        logger.debug("Restarting automation after auto-rejoin")
                        automation_manager.restart_automation("Auto-rejoin restart")
                        self._automation_was_running = False
                        logger.debug("Automation restarted successfully")
...
class RobloxRejoiner:
    def __init__(self, dig_tool_instance):
        ...
        self._rejoin_monitor_thread = None
        self._stop_rejoin_monitor = threading.Event()
    ...
    def get_server_url(self):
        ...
        if "roblox.com/share?" in server_link:
            if "code=" in server_link and "type=Server" in server_link:
                code_match = re.search(r"code=([^&]+)", server_link)
                type_match = re.search(r"type=([^&]+)", server_link)
                if code_match and type_match:
                    code = code_match.group(1)
                    type_val = type_match.group(1)
                    return (
                        f"roblox://navigation/share_links?code={code}&type={type_val}"
                    )
                else:
    ...
    def should_rejoin(self):
        ...
        with self._rejoin_lock:
            auto_rejoin_enabled = get_param(self.dig_tool, "auto_rejoin_enabled")
            has_server_link = get_param(self.dig_tool, "roblox_server_link")
            rejoin_interval = get_param(self.dig_tool, "rejoin_check_interval")
            time_since_last_rejoin = time.time() - self.last_rejoin_time
            can_rejoin = self.status_monitor.can_rejoin()
            automation_was_running = self.status_monitor._automation_was_running
            not_rejoining = not self._is_rejoining
            not_max_attempts = not self._max_attempts_reached

            return (
                not_rejoining
                and not_max_attempts
                and auto_rejoin_enabled
                and has_server_link
                and rejoin_interval > 0
                and time_since_last_rejoin >= self.min_rejoin_interval
                and can_rejoin
                and automation_was_running
            )
    ...
    def attempt_rejoin(self):
        ...
        try:
            ...
            if not (server_url := self.get_server_url()):
                self.status_monitor.send_discord_notification(
                    "❌ **Auto-rejoin failed**: No valid server URL configured",
                    color=0xFF4757,
                )
                return False
            ...
            if self.rejoin_attempts >= self._max_attempts:
                ...
                self.status_monitor.send_discord_notification(
                    f"❌ **Auto-rejoin stopped**: Maximum attempts ({self._max_attempts}) reached",
                    color=0xFF4757,
                )
                return False
            ...
            self.status_monitor.send_discord_notification(
                "🔄 **Auto-rejoining**: Attempting to rejoin server...",
                color=0xFFA502,
                include_user=False,
            )
            ...
            if self.launch_roblox(server_url):
                ...
                self.status_monitor.send_discord_notification(
                    f"🔄 **Auto-rejoin successful**: Roblox launched (attempt #{self.rejoin_attempts})",
                    color=0x2ED573,
                    include_user=False,
                )
                ...
                self.rejoin_attempts += 1
                self.status_monitor.send_discord_notification(
                    f"❌ **Auto-rejoin failed**: Could not launch Roblox (attempt #{self.rejoin_attempts})",
                    color=0xFF4757,
                )
            return False
        finally:
    ...
    def _start_connection_timeout(self):
        def check_connection():
            time.sleep(60)
            if (
                self._current_attempt_start
                and hasattr(self.status_monitor, "_recently_rejoined")
                and self.status_monitor._recently_rejoined
            ):
                self.status_monitor._recently_rejoined = False
                if (
                    self.rejoin_attempts < self._max_attempts
                    and not self._max_attempts_reached
                ):
                    threading.Thread(target=self.attempt_rejoin, daemon=True).start()

        threading.Thread(target=check_connection, daemon=True).start()

    def _rejoin_monitor_loop(self):
        from utils.config_management import get_param

        check_interval = 2.0

        while not self._stop_rejoin_monitor.is_set():
            try:
                auto_rejoin_enabled = get_param(self.dig_tool, "auto_rejoin_enabled")
                ...
                if auto_rejoin_enabled:
                    ...
                    if should_rejoin:
                        self.attempt_rejoin()
                        self._stop_rejoin_monitor.wait(15)
                    else:
                        self._stop_rejoin_monitor.wait(check_interval)
                else:
                    self._stop_rejoin_monitor.wait(check_interval)

            except Exception as e:
                logger.error(f"Error in rejoin monitor: {e}")
                self._stop_rejoin_monitor.wait(check_interval)
    ...
    def start_monitoring(self):
        ...
        if (
            self._rejoin_monitor_thread is None
            or not self._rejoin_monitor_thread.is_alive()
        ):
            self._stop_rejoin_monitor.clear()
            self._rejoin_monitor_thread = threading.Thread(
                target=self._rejoin_monitor_loop, daemon=True
            )
            self._rejoin_monitor_thread.start()

    def stop_monitoring(self):
        if hasattr(self.status_monitor, "stop"):
            self.status_monitor.stop()
        self._stop_rejoin_monitor.set()
        if self._rejoin_monitor_thread and self._rejoin_monitor_thread.is_alive():
            self._rejoin_monitor_thread.join(timeout=2)
